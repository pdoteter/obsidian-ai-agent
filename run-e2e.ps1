# Obsidian AI Agent E2E Playwright Test Runner (Windows PowerShell)
...
# Create local test config.yaml (pointing to test-vault)
$testConfig = @"
...
if (-not $env:OPENAI_API_KEY -or -not $env:OPENROUTER_API_KEY -or -not $env:TELOXIDE_TOKEN) {
    Write-Host "[E2E ERROR] Real API keys (OPENAI_API_KEY, OPENROUTER_API_KEY, TELOXIDE_TOKEN) must be set in your environment or .env file to run these live E2E tests!" -ForegroundColor Red
    Exit 1
}
...
Write-Host "[E2E] Launching Obsidian AI Agent bot in background..." -ForegroundColor Cyan
...
Write-Host "[E2E] Executing Playwright End-to-End Test Suite..." -ForegroundColor Green
$testError = $null
try {
    npx playwright test
...
if ($testError) {
    ...
    Write-Host "[E2E SUCCESS] E2E Playwright Run Completed Successfully!" -ForegroundColor Green
    Exit 0
}
