...
- [x] 3.2 Refactor `handle_text_message` in `src/handlers/text.rs` to use `AiService`
- [x] 3.3 Refactor `handle_voice_message` in `src/handlers/voice.rs` to use `AiService`
- [x] 3.4 Refactor `handle_url_message` and `handle_transcript_callback` in `src/handlers/url.rs` to use `AiService`
- [x] 3.5 Refactor photo handlers in `src/handlers/photo.rs`

## 4. Verification

- [x] 4.1 Run `cargo test` to ensure existing logic is preserved
- [x] 4.2 Add unit tests for `AiService` provider selection logic
- [x] 4.3 Verify provider switching works via `config.yaml` using a mock or stub provider
