...
- [ ] 1.1 Define the `AiProvider` trait in `src/ai/mod.rs`
- [ ] 1.2 Refine `AiError` in `src/error.rs` to support general provider errors
- [ ] 1.3 Update `Config` struct in `src/config.rs` to support the new provider-selection schema

## 2. Refactor Existing Clients

- [ ] 2.1 Refactor `OpenRouterClient` into `src/ai/providers/openrouter.rs` as an `AiProvider` implementation
- [ ] 2.2 Refactor `WhisperClient` into `src/ai/providers/openai_whisper.rs` as an `AiProvider` implementation
- [ ] 2.3 Implement the `AiService` orchestrator in `src/ai/mod.rs`

## 3. Handler Integration

- [ ] 3.1 Update `src/main.rs` to initialize the `AiService` and providers
