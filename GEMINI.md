...
- **Pull Requests**: Always run the complete End-to-End integration test suite (via `./run-e2e.ps1` and `./run-e2e-gemini.ps1`) before initiating, preparing, or submitting any Pull Request to ensure that all core WebUI, WebSocket, and Telegram capturing features operate successfully under live AI mock runs.
